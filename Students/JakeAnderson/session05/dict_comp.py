...
food = {"name": u"Jake",
              ...
              u"salad": u"greek",
              u"pasta": u"lasagna"}
print '%(name)s is from %(city)s, and he likes %(cake)s cake, %(fruit)s fruit, %(salad)s salad, and %(pasta)s pasta' % food


set= dict([(number, hex(number)) for number in (range(16))])
print set

food_new = dict([(k, v.count('a')) for k, v in food.items()])
print food_new
