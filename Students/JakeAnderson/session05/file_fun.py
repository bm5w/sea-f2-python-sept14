...
def time_to_trash_the_whitespaces(file_coming_in, file_goin_out):
...
def trashing_with_a_list_comp(file_coming_in, file_goin_out):
    #I'm using a list comprehesion for this one
     file_in = io.open(white_space.txt).readlines()
     file_out = [line.strip()+"\n" for line in file_in]
     io.open(file_goin_out, 's').writelines(file_out)
...
if __name__ == '__main__':
    ...
    time_to_trash_the_whitespaces(file_coming_in,file_goin_out)
